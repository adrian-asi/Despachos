...
// --- DATOS DEMO ---
const ESTADOS_OV = ['PENDIENTE', 'AGENDADA', 'URGENTE', 'VENCIDA', 'EMITIDA', 'PROGRAMADA', 'DESPACHADA', 'CERRADA', 'BLOQUEADO'];
...
function genOVs() {
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        const estado = i <= 5 ? 'BLOQUEADO' : i <= 30 ? pick(['PENDIENTE', 'URGENTE', 'VENCIDA', 'AGENDADA'])
            : i <= 60 ? 'EMITIDA'
                : i <= 80 ? 'PROGRAMADA'
                    : pick(['DESPACHADA', 'CERRADA']);
        const qty = seed(0, 10, 2000);

        const numItems = seed(0, 1, 3);
        const items = [];
        for (let j = 0; j < numItems; j++) {
            items.push({
                codigo: `P-${String(seed(0, 1, 9999)).padStart(4, '0')}`,
                descripcion: pick(['Válvula hidráulica', 'Freno neumático', 'Sensor de temperatura', 'Bomba centrífuga', 'Compresor industrial', 'Filtro de aceite', 'Rodamiento axial', 'Turbina de vapor']),
                cantidad: seed(0, 1, qty),
        ...
        ovs.push({
            ...
            pais: 'Perú',
            pieza_codigo: items[0].codigo,
            pieza_descripcion: items[0].descripcion,
            cantidad: qty,
            udm: items[0].udm,
            items: items,
...
function genHojas() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        hojas.push({
            ...
            numero: `HR-${45900 + i}`,
            fecha: fmtDate(d),
            turno: pick(['AM 1', 'AM 2', 'AM 3', 'PM 1', 'PM 2', 'PM 3']),
...
function badgeEstado(estado) {
    const map = {
        ...
        EMITIDA: 'badge-purple', PROGRAMADA: 'badge-yellow', DESPACHADA: 'badge-green', CERRADA: 'badge-gray', BLOQUEADO: 'badge-red',
